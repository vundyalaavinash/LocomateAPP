...
public class SlidingPaneLayout extends ViewGroup {
	 ...
	 * closed.
	 */
	private static int DEFAULT_OVERHANG_SIZE = 50;

	/**
	 * If no fade color is given by default it will fade to 80% gray.
	 */
	private static final int DEFAULT_FADE_COLOR = 0xffffff;
	 ...
	 */
	private Drawable mShadowDrawable = getResources().getDrawable(R.drawable.shadow);
	...
	void updateObscuredViewsVisibility(View panel) {
		...
		for (int i = 0, childCount = getChildCount(); i < childCount; i++) {
			...
			final int clampedChildRight = Math
					.min(rightBound, child.getRight());
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (lp.slideable) {
				...
				mSlideRange = PersistantAppData.w;
				lp.dimWhenOffset = xStart + lp.leftMargin + range + childWidth
						/ 2 > width - paddingRight;
				if(mSlideOffset>0.5){
					mSlideOffset=1.0f;
				}
		...
		mSlideRange = PersistantAppData.w - mOverhangSize ;
		mFirstLayout = false;
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		if (PersistantAppData.shouldSlide) {
	...
	public void incereaseSize() {
		mSlideableView.setLayoutParams(new LayoutParams(480,
				LayoutParams.MATCH_PARENT));
	}

	public boolean smoothSlideToFull(float slideOffset, int velocity) {
		if (!mCanSlide) {
			// Nothing to do.
			return false;
		}
		...
		final LayoutParams lp = (LayoutParams) mSlideableView.getLayoutParams();
		int x = (int) mSlideRange;
		if (mDragHelper.smoothSlideViewTo(mSlideableView, PersistantAppData.w,
				mSlideableView.getTop())) {
			setAllChildrenVisible();
			ViewCompat.postInvalidateOnAnimation(this);
			return true;
	...
	public boolean smoothSlideToBack(float slideOffset, int velocity) {
		if (!mCanSlide) {
			// Nothing to do.
			return false;
		}
		...
		// mSlideableView.setLayoutParams(new
		// LayoutParams(Persistance.w-150,LayoutParams.MATCH_PARENT));
		if (mDragHelper.smoothSlideViewTo(mSlideableView, x,
				mSlideableView.getTop())) {
			setAllChildrenVisible();
			ViewCompat.postInvalidateOnAnimation(this);
	...
	public void draw(Canvas c) {
		...
		mShadowDrawable.setBounds(left-50, top, right, bottom);
	...
	class AccessibilityDelegate extends AccessibilityDelegateCompat {
		...
		public void onInitializeAccessibilityNodeInfo(View host,
			...
			final AccessibilityNodeInfoCompat superNode = AccessibilityNodeInfoCompat
					.obtain(info);
			...
			final ViewParent parent = ViewCompat
					.getParentForAccessibility(host);
	...
	public void setDEFAULT_OVERHANG_SIZE(int i) {
		DEFAULT_OVERHANG_SIZE = i;
		mOverhangSize = (int) (DEFAULT_OVERHANG_SIZE * dens + 0.5f);		
		DEFAULT_OVERHANG_SIZE = 0;
	}
	
	public int getmOverhangSize(){
